...
@section('posts')
    ...
    <div class="h-full bg-white flex flex-col space-y-10 justify-center items-center">
        <div class="bg-white w-96 shadow-xl rounded p-5">
            ...
            <form action="{{ route('register-store') }}" class="space-y-5 mt-5" method="POST">
                @csrf
                <input name="name" value="{{ old('name') }}" type="text" class="w-full h-12 border border-gray-800 rounded px-3 @error('name') border-red-500 @enderror" placeholder="Имя" />
                ...
                <input name="password" value="{{ old('email') }}" type="password" class="w-full h-12 border border-gray-800 rounded px-3 @error('password') border-red-500 @enderror" placeholder="Пароль" />
